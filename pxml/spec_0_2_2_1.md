...
- Commandable — 64 bookended commands across 8 control axes give users prompt-time power over every aspect of the pipeline
...
Minimize output. No preamble, no caveats, just the answer. This controls *verbosity*, not state.
...
| Communication | `/format` `/tone` `/compact` `/compaction` `/verbose` `/ask` `/me` `/focus` | 8 |
...
| Agent (LLM) | `/think` `/selftalk` `/introspect` `/explain` `/reason` `/delegate` `/as` `/format` `/compact` `/verbose` `/focus` `/critique` `/test` `/review` `/compare` `/report` `/retry` `/iterate` `/research` `/investigate` `/implement` |
...
| 2026-04-08 | 0.2.2.1-draft | `/compaction` for state compression (distinct from `/compact` output brevity). `/validate` gains `target` parameter alongside `against`. `/run` as counterpart to `/step` (debugger resume). `/report` for natural-language post-mortems (distinct from `/abort` structured trace). `/extend` and `/without` gain structured parameters (`to`/`from`, `agents`, `tools`, `plugins`) alongside freetext body for precise + fuzzy modification. `/abort` documented as always recording structured trace data. `StateCompacted` event added. `CompilationRevised` event gains `revision_params` field. 64 commands total. |
