...
- Commandable — 49 bookended commands across 8 control axes give users prompt-time power over every aspect of the pipeline
...
Chunk boundaries: `<defaults>`, `<state>`, `<capability>`, `<pre>`, `<exec>`, `<post>`, `<with>` — each emitted as a complete AST node when its bookend closes.
...
Minimize output. No preamble, no caveats, just the answer.

**`/verbose = .../`**
Stage: Agent. Immediate.
Maximize detail. Options: `reasoning`, `all`.
...
**`/harness ... harness/`**
Stage: System. Nests.
Configure cross-cutting harness concerns directly.

```
/harness circuit_breaker = on, fallback = claude-haiku harness/
```
...
**`/validate ... validate/`**
Stage: Analyst. Nests.
Validate against a Gladius spec. Maps to the Validate interceptor plugin.

```
/validate against = ^output_schema validate/
```
...
**`/step/`**
Stage: System. Immediate.
Execute one agent at a time, pausing between each. The user must approve each step.
...
**`/abort/`**
Stage: System. Immediate.
Stop everything. Immediately. The `ExecutionFailed` event is recorded.
...
**`/extend ... extend/`**
Stage: Analyst. Nests.
Takes a previous compilation and adds to it. Creates a new compilation with `parent_id` pointing to the original. The Analyst merges the extension into the existing pipeline.

```
/extend pxml:a1b2c3d4
  Add @performance_reviewer to the parallel plan.
...
**`/without ... without/`**
Stage: Analyst. Nests.
Inverse of `/extend`. Takes a previous compilation and removes from it. The Analyst produces a new compilation with the specified subtractions.

```
/without pxml:a1b2c3d4
  Remove @style_check and lower all retries to 1.
without/
```
...
| `<capability>` | Metadata (A2A discovery) |
...
| `CompilationRevised` | `parent_id`, `revision`, `revision_type` (extend/without), `inline_text`, `block_text`, `block_ast`, `receipt`, `revised_at` |
...
| Axis | Commands | Count |
|------|----------|-------|
| Task framing | `/prompt` `/goal` `/context` `/assume` `/constraint` `/ignore` `/scope` `/pin` `/include` | 9 |
| Thinking | `/think` `/selftalk` `/introspect` `/explain` `/reason` | 5 |
| Delegation | `/agent` `/model` `/pick-tool` `/drop-tool` `/delegate` `/as` | 6 |
| Communication | `/format` `/tone` `/compact` `/verbose` `/ask` `/me` `/focus` | 7 |
| Protection | `/redact` `/cost` `/timeout` `/sandbox` `/audit` `/harness` | 6 |
| Verification | `/critique` `/validate` `/test` `/dry-run` `/review` `/compare` `/diff` | 7 |
| Memory | `/session` `/remember` `/forget` `/recall` `/snapshot` `/replay` `/alias` | 7 |
| Flow control | `/plan` `/step` `/checkpoint` `/retry` `/abort` `/rollback` `/iterate` `/research` `/investigate` `/implement` `/if` `/unless` `/extend` `/without` | 14 |
| **Total** | | **61** |
...
| Stage | Commands |
|-------|----------|
| Advocate (SLM) | `/prompt` `/goal` `/me` `/ask` `/tone` `/recall` `/pin` |
| Analyst (MLM) | `/context` `/assume` `/constraint` `/ignore` `/scope` `/include` `/agent` `/model` `/pick-tool` `/drop-tool` `/validate` `/plan` `/redact` `/sandbox` `/extend` `/without` |
| Agent (LLM) | `/think` `/selftalk` `/introspect` `/explain` `/reason` `/delegate` `/as` `/format` `/compact` `/verbose` `/focus` `/critique` `/test` `/review` `/compare` `/retry` `/iterate` `/research` `/investigate` `/implement` |
| System | `/cost` `/timeout` `/audit` `/harness` `/dry-run` `/diff` `/session` `/remember` `/forget` `/snapshot` `/replay` `/alias` `/step` `/checkpoint` `/abort` `/rollback` `/if` `/unless` |

### By form

| Form | Commands |
|------|----------|
| Nests | `/prompt` `/goal` `/context` `/assume` `/constraint` `/ignore` `/scope` `/pin` `/include` `/think` `/selftalk` `/introspect` `/explain` `/reason` `/agent` `/delegate` `/as` `/format` `/ask` `/me` `/focus` `/redact` `/cost` `/sandbox` `/harness` `/critique` `/validate` `/test` `/dry-run` `/review` `/compare` `/diff` `/session` `/remember` `/forget` `/recall` `/alias` `/plan` `/checkpoint` `/retry` `/iterate` `/research` `/investigate` `/implement` `/if` `/unless` `/extend` `/without` |
| Immediate | `/model` `/pick-tool` `/drop-tool` `/tone` `/compact` `/verbose` `/timeout` `/audit` `/snapshot` `/replay` `/step` `/abort` `/rollback` |
...
| 2026-04-08 | 0.2.2-draft | AAA pipeline (Advocate → Analyst → Agent). 49-command vocabulary across 8 axes with bookended `/cmd ... cmd/` syntax. Compilation receipts with `pxml:shortid` references. `/if` and `/unless` conditional guards (not Turing-complete). `/extend` and `/without` for pipeline composition and subtraction. `/pin` for AAA bypass. `/alias` and `/include` for reusable patterns. `/diff` for revision comparison. Backtick support for inline code and fenced code blocks. Open questions on command parameter syntax, conditional expressions, alias scoping, and AAA boundaries. |
